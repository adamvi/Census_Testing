...
state_acct_data_18[,
  ...
  # SQSS_Score := rowMeans(.SD, na.rm = TRUE),   # SQSS = NA if ALL are NA, mean of those available
  # SQSS_Score := rowSums(.SD, na.rm = FALSE)/3, # SQSS = NA if any are NA?
  SQSS_Score := rowSums(.SD, na.rm = TRUE)/3,    # Treated as 0? 
  .SDcols = c("Beyond_The_Core", "Literacy", "Student_Attendance")
]#[,
#   c("Beyond_The_Core", "Literacy", "Student_Attendance") := NULL
# ]
...
state_acct_data_19[,
  SQSS_Score := rowSums(.SD, na.rm = TRUE)/3,
  .SDcols = c("Beyond_The_Core", "Literacy", "Student_Attendance")
]
...
sch_smry_0E <- state_acct_data_18[sch_smry_0]
...
assign(paste0("sch_smry_0E_", min.n, "N"), tmp.filtered)
...
# * ProgELP_Z = (ProgELP_Score - ProgELP_Mean)/ProgELP_SD
# * SQSS_Z = (SQSS_Score - SQSS_Mean)/SQSS_SD
...
P5_Lookup <-
  sch_smry_0E_10N[,
    .(P5 = quantile(x = SumScore, probs = 0.05)),
    keyby = "Group"
]
...
if (!dir.exists("./Data/Accountability_Ratings"))
     dir.create("./Data/Accountability_Ratings")

#  Read in the custom function
source("../functions//accountabilityModel.R")

#  Calculate and output
fprefix <- "./Data/Accountability_Ratings/School_AcctRatings_Condition_"

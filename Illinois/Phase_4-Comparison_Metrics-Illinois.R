...
# XI. Compute the same summary statistics for all records (i.e., unfiltered)
  # XI.a
    full_stu_n <-
        tmp_full[,
  ...
  for (cond in c("0", "1a", "1b", "1c", "2", "3", "4")) {
    ...
    if (cond != "0") {
      ...
      # ATSI - 15. & 16.
        ...
        ach.scat <-
            ggplot(tmp_data, aes(x = ACH_Z.x, y = ACH_Z.y)) + theme_bw() +
            geom_hex(bins = 50, show.legend = FALSE) + xlim(c(-3, 3)) + ylim(c(-3, 3)) +
            scale_fill_gradient(low = "grey", high = "black", na.value = NA) +
            geom_smooth(method = "gam", se = FALSE, color = "red") +
            labs(x = paste("Achievement Z - Condition 0, Min N =", min.n),
                y = paste("Achievement Z - Condition", cnd.nm, ", Min N =", min.n)) +
            ggtitle("Focus vs Reference Condition") +
            theme(plot.title = element_text(hjust = 0.5, size=14))
        ach.cond <-
            ggplot(tmp_data, aes(x = ACH_Z.x, y = (ACH_Z.x - ACH_Z.y))) + theme_bw() +
            geom_hex(bins = 50, show.legend = FALSE) + xlim(c(-3, 3)) + ylim(c(-1.5, 1.5)) +
            scale_fill_gradient(low = "grey", high = "black", na.value = NA) +
            geom_smooth(method = "gam", se = FALSE, color = "red") +
            labs(x = paste("Achievement Z - Condition 0, Min N =", min.n),
                y = paste("Achievement Z Conditional Difference", "( 0 -", cnd.nm, ")")) +
            ggtitle("Conditional Difference") +
            theme(plot.title = element_text(hjust = 0.5, size=14))

        oth.scat <-
            ggplot(tmp_data, aes(x = Other_Z.x, y = Other_Z.y)) + theme_bw() +
            geom_hex(bins = 50, show.legend = FALSE) + xlim(c(-3, 3)) + ylim(c(-3, 3)) +
            scale_fill_gradient(low = "grey", high = "black", na.value = NA) +
            geom_smooth(method = "gam", se = FALSE, color = "red") +
            labs(x = paste(
        ...
        oth.cond <-
            ggplot(tmp_data, aes(x = Other_Z.x, y = (Other_Z.x - Other_Z.y))) + theme_bw() +
            geom_hex(bins = 50, show.legend = FALSE) + xlim(c(-3, 3)) + ylim(c(-1.5, 1.5)) +
            scale_fill_gradient(low = "grey", high = "black", na.value = NA) +
            geom_smooth(method = "gam", se = FALSE, color = "red") +
            labs(x = paste(
        ...
        sumsc.scat <-
            ggplot(tmp_data, aes(x = SumScore.x, y = SumScore.y)) + theme_bw() +
            geom_hex(bins = 50, show.legend = FALSE) + xlim(c(-3, 3)) + ylim(c(-3, 3)) +
            scale_fill_gradient(low = "grey", high = "black", na.value = NA) +
            geom_smooth(method = "gam", se = FALSE, color = "red") +
            labs(x = paste("Sum Score Z - Condition 0, Min N =", min.n),
                y = paste("Sum Score Z - Condition", cnd.nm, ", Min N =", min.n))
        sumsc.cond <-
            ggplot(tmp_data, aes(x = SumScore.x, y = (SumScore.x - SumScore.y))) + theme_bw() +
            geom_hex(bins = 50, show.legend = FALSE) + xlim(c(-3, 3)) + ylim(c(-1.5, 1.5)) +
            scale_fill_gradient(low = "grey", high = "black", na.value = NA) +
            geom_smooth(method = "gam", se = FALSE, color = "red") +
            labs(x = paste("Sum Score Z - Condition 0, Min N =", min.n),
...
###  Threshold for Comparison Metrics
#
# As recommended by the technical advisors, perform bootstrap resampling of the
# full set of accountability measures with the full population of schools in the
# reference conditions (i.e., sampling condition 0 with minimum n-count of 10).
# This allows us to estimate the expected degree of variability from random
# samples of schools, providing a threshold for the amount of difference in the
# comparison metrics between conditions that would be considered unusual
# (vs. expected). In other words, it helps answer the question,"How much of a
# departure for a comparison metric is unusual?" Threshold should be estimated
# for these comparison metrics:
#   * Match rates, false positive rates and false negative rates for CSI and
#     ATSI identification
...
rm(Metric_Bootstrap)
load("./Data/Phase_4-Metric_Bootstrap/Phase_4/Metric_Bootstrap_2019.rda")
Metric_Bootstrap_2019 <- Metric_Bootstrap
rm(Metric_Bootstrap)

Bootstrap_Comparisons <- list(
    CSI = rbindlist(list(
        Metric_Bootstrap_2018[["CSI"]],
        Metric_Bootstrap_2019[["CSI"]]
    )),
    ATSI = rbindlist(list(
        Metric_Bootstrap_2018[["ATSI"]],
        Metric_Bootstrap_2019[["ATSI"]]
    )),
    DIST = rbindlist(list(
        Metric_Bootstrap_2018[["DIST"]],
        Metric_Bootstrap_2019[["DIST"]]
    ))
  )

# 27. For a given comparison metric, C, let C_Mean and C_SD be the mean and
#     standard deviation of the metric across 100 replications respectively.
#     The comparison threshold for C is estimated as (C_Low, C_High) where
#   a. C_Low = C_Mean - 2 (C_SD/10)
#   b. C_High = C_Mean + 2 (C_SD/10)
#     For example, for the %_Match of identified CSI schools, C_Mean is the
#     average %_Match values across 100 replications. C_SD is the standard
#     deviation of %_Match values across 100 values. C_Low and C_High are
#     calculated accordingly.

csi.boot.table <-
    Bootstrap_Comparisons[["CSI"]][,
        .(`C_Low_%_Match` =
            mean(`%_Match`) - (2 * (sd(`%_Match`)/10)),
          `C_High_%_Match` =
            mean(`%_Match`) + (2 * (sd(`%_Match`)/10)),
          `C_Low_%_FalsePos` =
            mean(`%_FalsePos`) - (2 * (sd(`%_FalsePos`)/10)),
          `C_High_%_FalsePos` =
            mean(`%_FalsePos`) + (2 * (sd(`%_FalsePos`)/10)),
          `C_Low_%_FalseNeg` =
            mean(`%_FalseNeg`) - (2 * (sd(`%_FalseNeg`)/10)),
          `C_High_%_FalseNeg` =
            mean(`%_FalseNeg`) + (2 * (sd(`%_FalseNeg`)/10))
        ),
        keyby = "Year"
...
dist.boot.table <-
    Bootstrap_Comparisons[["DIST"]][,
        .(C_Low_ACH_Correl =
            mean(ACH_Correl) - (2 * (sd(ACH_Correl)/10)),
          C_High_ACH_Correl =
            mean(ACH_Correl) + (2 * (sd(ACH_Correl)/10)),
          C_Low_ACH_RMSE =
            mean(ACH_RMSE) - (2 * (sd(ACH_RMSE)/10)),
          C_High_ACH_RMSE =
            mean(ACH_RMSE) + (2 * (sd(ACH_RMSE)/10)),

          C_Low_Other_Correl =
            mean(Other_Correl) - (2 * (sd(Other_Correl)/10)),
          C_High_Other_Correl =
            mean(Other_Correl) + (2 * (sd(Other_Correl)/10)),
          C_Low_Other_RMSE =
            mean(Other_RMSE) - (2 * (sd(Other_RMSE)/10)),
          C_High_Other_RMSE =
            mean(Other_RMSE) + (2 * (sd(Other_RMSE)/10)),

          C_Low_SumScore_Correl =
            mean(SumScore_Correl) - (2 * (sd(SumScore_Correl)/10)),
          C_High_SumScore_Correl =
            mean(SumScore_Correl) + (2 * (sd(SumScore_Correl)/10)),
          C_Low_SumScore_RMSE =
            mean(SumScore_RMSE) - (2 * (sd(SumScore_RMSE)/10)),
          C_High_SumScore_RMSE =
            mean(SumScore_RMSE) + (2 * (sd(SumScore_RMSE)/10))
        ),
        keyby = "Year"
    ]
...
##  Bootstrap results
titl.sty <-
  createStyle(textDecoration = "Bold",  border = "top")
tmp.sheet <- "Bootstrap Results"
addWorksheet(phase_4_wb, tmp.sheet)
addStyle(phase_4_wb, tmp.sheet, style = titl.sty, cols = 2, rows = 2)
writeData(
  phase_4_wb, tmp.sheet,
  x = "CSI Bootstrap Confidence Intervals",
  startCol = 2, startRow = 2
)
writeData(
  phase_4_wb, tmp.sheet, round(csi.boot.table[, Year := as.numeric(Year)], 3),
  startCol = 2, startRow = 3, borders = "surrounding", headerStyle = hs1
)

addStyle(phase_4_wb, tmp.sheet, style = titl.sty, cols = 2, rows = 9)
writeData(
  phase_4_wb, tmp.sheet,
  x = "ATSI Bootstrap Confidence Intervals",
  startCol = 2, startRow = 9
)
writeData(
  phase_4_wb, tmp.sheet, round(atsi.boot.table[, Year := as.numeric(Year)], 3),
  startCol = 2, startRow = 10, borders = "surrounding", headerStyle = hs1
)

writeData(
  phase_4_wb, tmp.sheet,
  x = "Indicator Correlation and RMSE Bootstrap Confidence Intervals",
  startCol = 2, startRow = 16
)
addStyle(phase_4_wb, tmp.sheet, style = titl.sty, cols = 2, rows = 16)
writeData(
  phase_4_wb, tmp.sheet, round(dist.boot.table[, Year := as.numeric(Year)], 3),
  startCol = 2, startRow = 17, borders = "surrounding", headerStyle = hs1
)

##    Save Workbook
saveWorkbook(
  phase_4_wb,
  "./Data/Phase_4-Comparison_Metrics/Comparison_Metrics.xlsx",
  overwrite = TRUE
)

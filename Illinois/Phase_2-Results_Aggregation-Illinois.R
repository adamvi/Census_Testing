...
###############################################################################
####                                                                       ####
####    2018 and 2019 Growth and Achievement Aggregations for Illinois     ####
####                                                                       ####
###############################################################################
require(data.table)
...
if (!dir.exists("./Data/Phase_2-School_Summaries"))
     dir.create("./Data/Phase_2-School_Summaries")
...
fwrite(school_aggregation_all_students,
    file = "Data/Phase_2-School_Summaries/School_Condition_AllGrowth_AllStudents_Illinois_AllYears_AVI.csv"
)
...
sch_summary_cnd_1a <-
    sch_summary_cnd_1a[YEAR == 2019] # c(2018, 2019)
...
fwrite(sch_summary_cnd_1a[YEAR == 2018][, YEAR := NULL],
    file = "Data/Phase_2-School_Summaries/School_Condition_1a_Illinois_2018_AVI.csv"
)
fwrite(sch_summary_cnd_1a[YEAR == 2019][, YEAR := NULL],
    file = "Data/Phase_2-School_Summaries/School_Condition_1a_Illinois_2019_AVI.csv"
)

#' Note that the above code can be accomplished with the `cond1aAggrGator`
#' function:
sch_smry_cnd_1a <-
    cond1aAggrGator(data_table = Illinois_Data_LONG, years = 2019)
...
fwrite(school_aggregation_student_demogs,
    file = "Data/Phase_2-School_Summaries/School_Condition_AllGrowth_Demographics_Illinois_AllYears_AVI.csv"
)
...
##  School_Condition_<c> _<State>_<FYear>_ <init>.csv
    if (nrow(tmp.tbl) > 0L) {
      fwrite(
        x = tmp.tbl,
        file = paste0(fprefix, cond, "_Illinois_", yr, "_AVI.csv")
      )
    }
  }
}

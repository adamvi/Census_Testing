...
###############################################################################
####                                                                       ####
####      Create 2018 and 2019 SGPs for Mississippi - All Conditions       ####
####                                                                       ####
###############################################################################
...
#'
#' ## Load SGP package and Mississippi data
#'
...
if (!exists("Mississippi_Data_LONG")) {
  source("../functions/freadZIP.R")
  Mississippi_Data_LONG <-
    freadZIP(
      "Data/Cleaned_Data/Student_LongTestData_Mississippi_2016-2022_AVI.csv.zip"
    )[YEAR < 2020]
} else {
  Mississippi_Data_LONG <- Mississippi_Data_LONG[YEAR < 2020]
}

ms.knots.bounds <-
  SGP::createKnotsBoundaries(Mississippi_Data_LONG[YEAR %in% 2017:2018])

SGPstateData[["MS"]] <- NULL
SGPstateData[["MS"]][["Achievement"]][["Knots_Boundaries"]] <- ms.knots.bounds
...
#' we read these in and combine them into a single list object,
#' `mississippi.config`, that will be supplied to the `abcSGP` function.
#' 
...
Mississippi_SGP <-
    abcSGP(
        sgp_object = Mississippi_Data_LONG,
        # state = "MS",
        steps = c("prepareSGP", "analyzeSGP", "combineSGP"),
        sgp.config = mississippi.config,
...
rm(Mississippi_Data_LONG)
Mississippi_Data_LONG <- copy(Mississippi_SGP@Data)

setnames(x = Mississippi_Data_LONG, old = "SGP", new = "SGP_Cnd_0")
Mississippi_Data_LONG[,
...
Condition_0_CoefMatrices <- copy(Mississippi_SGP@SGP[["Coefficient_Matrices"]])
...
setnames(x = Mississippi_Data_LONG, old = "SGP", new = "SGP_Cnd_1b")
Mississippi_Data_LONG[,
    c("SGP_NORM_GROUP", "SGP_NORM_GROUP_SCALE_SCORES",
      "SCALE_SCORE_PRIOR", "SCALE_SCORE_PRIOR_STANDARDIZED"
    ) := NULL
...
Condition_1b_CoefMatrices <- copy(Mississippi_SGP@SGP[["Coefficient_Matrices"]])
...
#' conditions 1b and 2. The data object `Mississippi_Data_LONG` now includes the
...
setnames(x = Mississippi_Data_LONG, old = "SGP", new = "SGP_Cnd_1c")
Mississippi_Data_LONG[,
    c("SGP_NORM_GROUP", "SGP_NORM_GROUP_SCALE_SCORES",
      "SCALE_SCORE_PRIOR", "SCALE_SCORE_PRIOR_STANDARDIZED"
    ) := NULL
...
Condition_1c_CoefMatrices <- copy(Mississippi_SGP@SGP[["Coefficient_Matrices"]])
...
#'
#' The call to the`abcSGP` function here is identical to that made for
#' conditions 1b and 1c. The data object `Mississippi_Data_LONG` now includes the
#' results from conditions 0 through 1c, and the configuration object,
#' `mississippi.config`, has been updated.
#'
...
setnames(x = Mississippi_Data_LONG, old = "SGP", new = "SGP_Cnd_2")
Mississippi_Data_LONG[,
    c("SGP_NORM_GROUP", "SGP_NORM_GROUP_SCALE_SCORES",
...
Condition_2_CoefMatrices <- copy(Mississippi_SGP@SGP[["Coefficient_Matrices"]])
...
save("Mississippi_Data_LONG", file = "Data/Student_Growth/Mississippi_Data_LONG.rda")

fname <- "Data/Student_Growth/Student_LongTestData_Mississippi_2016-2019_AVI.csv"
